...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (empty($errors) && !empty($_COOKIE[session_name()]) &&
        ...
        $values['fullname'] = strip_tags($row['fullname_value']);
        $values['telephone'] = strip_tags($row['telephone_value']);
        $values['email'] = strip_tags($row['email_value']);
        $values['birthday'] = strip_tags($row['birthday_value']);
        $values['gender'] = strip_tags($row['gender_value']);
        $values['biography'] = strip_tags($row['biography_value']);
        $values['checkk'] = strip_tags($row['checkk_value']);
        $values['language'] = findAllLanguagesByClient($db, $row['id']);
...
function restoreValues($db) {
    $values = array();
    $values['fullname'] = empty($_COOKIE['fullname_value']) ? '' : strip_tags($_COOKIE['fullname_value']);
